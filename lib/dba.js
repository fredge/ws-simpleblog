...
exports.entryPost = function(data, callback) {
    var sql = 'insert into posts (body, enrty_date) values (?, now())';
...
exports.getNewerPosts = function(callback) {
    var sql = 'select * from posts where id > ? order by id desc';
    var params = [data.id];
    executeQuery(sql, params, function(err, results) {
        callback(err, results);
    });
...
// test code
//executeQuery("insert into posts (body, entry_date) values('sample post 3', now())", null, function(err, result) {
//    if (err) {
//        console.log(err);
//        return;
//    }
